    ...
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <div className="mx-auto w-full max-w-5xl flex-1 px-5 pb-16 pt-8 md:px-8">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-neutral-900 md:text-3xl">Inventory</h1>
          <button
        ...
        <div className="mt-6 flex flex-col gap-2">
          {products.map((p) => {
            ...
            return (
              ...
              >
                <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl bg-neutral-50">
          ...
          {products.length === 0 && (
            <p className="py-10 text-center text-sm text-neutral-400">No watches yet.</p>
          )}
        </div>
      </div>
    </div>
...
function ProductForm({
  ...
  return (
    <form
      ...
      className="mt-4 flex flex-col gap-4 rounded-2xl border border-neutral-200 p-5"
    >
      <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        <button
          ...
          className="flex h-20 w-20 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl border border-dashed border-neutral-300 bg-neutral-50"
        >
          {imagePreview ? (
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Dial color">
          <input
            ...
            className="h-9 w-full rounded-lg border border-neutral-200"
          />
        </Field>
        <Field label="Strap type">
          <select
            value={fields.strapType}
            onChange={(e) => set("strapType", e.target.value)}
            className="w-full rounded-lg border border-neutral-200 px-3 py-2 text-sm outline-none"
          >
            {STRAP_TYPES.map((s) => (
        ...
        <Field label="Case shape">
          <select
            value={fields.caseShape}
            onChange={(e) => set("caseShape", e.target.value)}
            className="w-full rounded-lg border border-neutral-200 px-3 py-2 text-sm outline-none"
          >
            {CASE_SHAPES.map((s) => (
              <option key={s} value={s}>
      ...
      <button
        type="submit"
        disabled={submitting}
        className="rounded-full bg-neutral-900 py-3 text-sm font-semibold text-white disabled:opacity-50"
      >
